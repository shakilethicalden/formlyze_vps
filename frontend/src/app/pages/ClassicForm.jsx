  ...
  const categories = useMemo(
    () =>
      [
        'all',
        ...new Set(
          template.map(item => item.category).filter(cat => cat != null)
        )
      ].sort(),
    []
  )
    ...
    <div className='lg:px-8 px-4 lg:pr-8 mt-28  '>
      ...
      <div className='flex flex-col md:flex-row gap-4 mb-8 p-4 bg-third rounded-lg shadow-sm'>
        ...
        <div className='flex gap-2'>
          ...
          <button className='bg-[#1A1466] text-white px-4 py-2 rounded-md hover:bg-opacity-90 transition-colors'>
            Filter
          </button>
        </div>
      </div>
